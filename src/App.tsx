...
import {
  ...
  Typography,
  Button,
  Grid, // Import Grid v2 and alias it
  Card,
  CardMedia,
  CardContent,
  GlobalStyles,
        ...
        >
          {featuredProjects.map((project) => {
            ...
            return (
              <Box 
                key={project.id} 
                // Store ref to this card's DOM element for the observer
                ref={(el) => {
                  if (el) videoCardRefs.current.set(project.id, el);
                  else videoCardRefs.current.delete(project.id);
                }}
                data-id={project.id} // Add data-id to identify in observer
                sx={{ 
                  minWidth: cardWidths.min, 
                  maxWidth: cardWidths.max,
                  scrollSnapAlign: { xs: 'center', md: 'none' }, // Only snap on mobile
                }}
              >
                <ProjectCard
                  project={project}
